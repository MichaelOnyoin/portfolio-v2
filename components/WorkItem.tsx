...
const WorkItem: React.FC<WorkItemProps> = ({ src, alt, description, title, link, github }) => {
  return (
    ...
    <div className="flex flex-col w-full max-md:ml-0 max-md:w-full group hover:cursor-pointer relative ">
      <div className="flex flex-col grow justify-center px-12 py-16 w-full rounded-2xl backdrop-blur-[2px] bg-zinc-700 max-md:px-5 max-md:mt-10">
        <img loading="lazy" src={src} alt={alt} className="w-full aspect-[1.67]" />
      </div>
      <div className='absolute bottom-0 left-0 right-0 bg-[rgba(0,0,0,0.5)] p-4 mx-2 mb-4 float rounded-lg hidden group-hover:block'>
